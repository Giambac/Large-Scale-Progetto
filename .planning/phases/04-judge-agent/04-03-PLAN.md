...
<!-- V-4-01 fix: _NullNamer.name_cluster() now returns dict[str, str] with "name" and
     "description" keys, matching the ClusterNamer Protocol that build_initial_clustering_state()
     expects. build_initial_clustering_state() accesses naming_result["name"] and
     naming_result["description"] — returning a plain string caused TypeError at runtime.
     V-4-07 fix: Dead import of AnthropicClusterNamer removed — it was never used in
     run_baseline() and would cause ImportError in isolated test environments. -->
...
```
  ...
  <acceptance_criteria>
    ...
    - src/judge.py does NOT import AnthropicClusterNamer (removed dead import per V-4-07)
    - python -c "from src.judge import run_baseline; import inspect; src = inspect.getsource(run_baseline); assert 'AnthropicClusterNamer' not in src; print('no dead import ok')" exits 0
  </acceptance_criteria>
</task>
...
5. `python -c "from src.judge import run_baseline; import inspect; src = inspect.getsource(run_baseline); assert 'AnthropicClusterNamer' not in src; print('no dead import ok')"` exits 0
</verification>
...
- _NullNamer.name_cluster() returns dict with "name" and "description" keys (not a plain string)
- AnthropicClusterNamer is NOT imported in run_baseline() (dead import removed)
